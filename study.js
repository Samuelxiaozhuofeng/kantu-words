...
export async function renderStudy(view, id) {
  ...
  $('#modeSeg').onclick = e => { const m = e.target.dataset.mode; if (m && m !== mode) { settings.set({ ...settings.get(), studyMode: m }); renderStudy(view, id); } };
  ...
  function drawHint(force) {
    const it = items[i], on = force || hintMode === 'always' || (hintMode === 'wrong' && wrong > 0);
    $('#hint').textContent = on ? hintOf(it) : '···';
    $('#hint').className = 'hint' + (on ? '' : ' blank');
  }
  function show() {
    const it = items[i], ok = done.has(i);
    ...
    if (mode === 'tap') {
      for (const b of $('#stage').querySelectorAll('.box')) {
        const k = +b.dataset.i;
        b.className = 'box' + (done.has(k) ? ' ok' : '') + (k === i && (ok || revealed) ? ' sel' : '');
      }
  ...
  // 换题：选项从本课别的词里抽 3 个当干扰项（按显示文字去重，识别重复框同一个词时不会出两个正确答案）
  function newQuestion() {
    wrong = 0; revealed = false; bad = new Set();
    if (pick) {
      const it = items[i], seen = new Set([label(it)]);
      const others = shuffle(items.filter(x => x !== it && !seen.has(label(x)) && seen.add(label(x))));
