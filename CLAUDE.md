...
- **数据模型**（IndexedDB `kantu` 库 `lessons` 表，`lib.js`）：`{ id, title, image: Blob(JPEG≤1600px), items: [{ en, zh, ipa, pos, alts: [], box: [ymin, xmin, ymax, xmax], sent?, sentZh? }], created, lang?, folder? }`。`folder` 是课程文件夹：可选字符串，缺省 / 空串当未分组，内置课不写，读一律用 `folderOf(lesson)`，不按 `FOLDERS` 校验（老数据改名不丢）。`sent` / `sentZh` 是例句和它的中文（可选，缺省 `''`）：识别时勾了「生成例句」才要，或编辑页「补例句」按 `en` 匹配回填（没配上的保留原句）；学习页「听句子点图」只在题单里有句子时出现。`lang` 是课程语种（`lib.js` 的 `LANGS` 表：en/ja/ko/fr/de/es），缺省当 `en`（老课、内置课、旧备份），一律用 `langOf(lesson)` 取；**`en` 字段名不改**，存的是该语种的词。`box` 是 0–1000 归一化坐标，顺序是 **y 在前**（跟 Gemini 输出一致），`editor.js` 的 `boxStyle` 负责转 CSS。导出/导入 JSON 时 `image` 转 data URL。
- **设置**全在 `localStorage.kantu` 一个 JSON 里（`settings.get/set`），键：`apiUrl apiKey visionModel imageApiUrl imageApiKey imageModel voice hintMode studyMode homeTab packsAdded packLang lastLang wrong ankiDeck ankiPreset ankiClear withSent packsVer genParallel`。`wrong` 是错题本 `{ "课程id|词": 加入时间戳 }`，只在学习页结果页那一刻写、首页错题本标签删，**不进 IndexedDB 课程对象**（练习页拿的是课程快照，写回会盖掉编辑页的改动）；读一律用 `lib.js` 的 `wrongEntries(lessons, book)`，它按现有课程过滤。
