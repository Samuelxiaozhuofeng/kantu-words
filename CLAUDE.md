...
- **设置**全在 `localStorage.kantu` 一个 JSON 里（`settings.get/set`），键：`apiUrl apiKey visionModel imageApiUrl imageApiKey imageModel voice hintMode studyMode homeTab packsAdded packLang lastLang wrong ankiDeck ankiPreset ankiClear withSent packsVer genParallel folders`。`wrong` 是错题本 `{ "课程id|词": 加入时间戳 }`，只在学习页结果页那一刻写、首页错题本标签删，**不进 IndexedDB 课程对象**（练习页拿的是课程快照，写回会盖掉编辑页的改动）；读一律用 `lib.js` 的 `wrongEntries(lessons, book)`，它按现有课程过滤。
