// PWA：静态文件网络优先、失败用缓存；/tts、/packs（进库后就不需要了）、/dav（同步必须拿真远端，拿缓存会按旧快照合并）和外部接口不缓存
const C = 'kantu-v3';
...
self.addEventListener('fetch', e => {
  ...
  if (e.request.method !== 'GET' || u.origin !== location.origin || u.pathname.startsWith('/tts') || u.pathname.startsWith('/dav') || u.pathname.startsWith('/packs/')) return;
